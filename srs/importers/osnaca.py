"""Import the OSNACA data and metadata workbooks."""
...
def _to_float(value) -> float | None:
    """Read a number from a spreadsheet cell, ignoring trailing notes."""
...
def run_import(import_id: int) -> ReferenceImport:
    """Run one stored reference import and save its final status."""
...
def seed_lookups(meta_wb) -> None:
    """Update the lookup tables used by the reference library."""
...
def load_deposits(meta_wb, import_row, stats) -> dict[str, ReferenceDeposit]:
    """Import deposit locations and return them by code."""
...
def load_samples(meta_wb, import_row, deposits_by_code, stats, errors):
    """Import sample details from both metadata sheets."""
...
def _build_sample_metadata(row, minerals_by_code: dict) -> dict:
    ...
    minerals = [
        ...
        for code in mineral_codes
        if code in minerals_by_code
    ]
...
def load_measurements(data_wb, import_row, samples_by_code, stats, errors):
    """Import measurements from each supported data sheet."""
                ...
                errors.append(
                    {
                        ...
                        "reason": "No matching sample in the metadata workbook.",
                    }
                )
                continue
